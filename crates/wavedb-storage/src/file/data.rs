...
//! steady state: one SplitMix64 mix plus a mask.
//!
//! ## Rebalance (growth)
//!
//! `page_count` doubles when fill pressure crosses a threshold.  Rebalance
//! runs in the foreground (single-flight via `rebalancing` flag) and:
//!
...
//!
//! `page_count` is **always a power of two** so the modulo in `PageKey::page`
//! lowers to a single mask instruction.  Default initial count: 256.
...
impl DataFile {
    ...
    pub fn open_with_mode(
        ...
        // Create parent directory only when the file is meant to land on
        // disk — InMemory mode treats `path` as a label and shouldn't
    ...
    /// Format: a single postcard-encoded blob containing the page-count
    /// header plus every page.  The journal pipe provides the
    /// fine-grained crash-recovery story between snapshots.
    ...
    pub fn flush(&self) -> StorageResult<()> {
        ...
        let blob = postcard::to_allocvec(&(page_count, &*pages))?;
    ...
    fn reload_from_disk(&self) -> StorageResult<()> {
        ...
        if !page_count.is_power_of_two() || pages.len() as u64 != page_count {
            return Err(crate::StorageError::Other(format!(
        ...
        // Rebuild page_bytes from each loaded page's directory.
    ...
    fn fallback_index(primary: usize, key: PageKey, page_count: u64) -> usize {
        ...
        let mask = (page_count as usize).saturating_sub(1);
        (primary + step) & mask
    }
    ...
    /// Double `page_count` and rehash every entry that ends up on a
    /// different slot under the new size.
    ///
    /// Steps:
    ///
    ...
    pub fn rebalance(&self) -> StorageResult<()> {
        ...
        let result = (|| -> StorageResult<()> {
            ...
            let new_count = old_count.saturating_mul(2);
...
mod tests {
    ...
    /// Manual `rebalance()` doubles `page_count` and every existing
    /// entry remains findable afterwards.
    #[test]
    fn rebalance_doubles_page_count_and_preserves_entries() {
        let dir = tempfile::tempdir().unwrap();
        ...
        assert!(
            after >= before * 2,
            "expected at least doubling: {before} -> {after}"
        );
